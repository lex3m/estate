...
class ExtMinScript extends CClientScript {
	...
	public function registerScript($id,$script,$position=null,array $htmlOptions=array(), $isMin = false)
	{
		if (param('useMinify', false) && $isMin && ($position == self::POS_BEGIN || $position == self::POS_END || $position == self::POS_HEAD)) {
			// return file
			# registerScriptFile($url,$position=null,array $htmlOptions=array())
			# registerScript($id,$script,$position=null,array $htmlOptions=array())

			$fileGenName = 'generate-'. Yii::app()->language. '-'. $id . '.js';

			$mdFile = '';
			$md5Script = md5($script);
			$fileContent = @file_get_contents(Yii::app()->controller->assetsGenPath.DIRECTORY_SEPARATOR.$fileGenName);

			if ($fileContent) {
				$mdFile = md5($fileContent);
			}
			...
			{
				$value=$htmlOptions;
				$value['src']=$fileGenName;
			}
			$this->scriptFiles[$position][$fileGenName]=$value;
