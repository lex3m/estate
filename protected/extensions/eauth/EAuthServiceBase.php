...
abstract class EAuthServiceBase extends CComponent implements IAuthService {
	...
	protected function makeRequest($url, $options = array(), $parseJson = true) {
		...
		if ($headers['http_code'] != 200) {
			...
			throw new EAuthException(Yii::t('module_socialauth', 'Invalid response http code: {code}.', array('{code}' => $headers['http_code'])), $headers['http_code']);
		}
	...
	protected function parseJson($response) {
		try {
			...
			if (!isset($result)) {
				throw new EAuthException(Yii::t('module_socialauth', 'Invalid response format.', array()), 500);
			}
			else if (isset($error)) {
